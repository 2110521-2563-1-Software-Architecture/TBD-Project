...
function CreatePost(props) {
  ...
  const submit = () => {
    ...
    if (isEdit) {
      ...
      PostService.updatePost(sendToBackend)
        .then((response) => {
      ...
      PostService.createPost(sendToBackend)
        .then((response) => {
          if (response.data.status === 'success.') {
            setText('');
            setPhoto('');
  ...
  return (
    <div>
      ...
      <Modal
        ...
        onCancel={() => {
          setVisible(false);
          if (isEdit) setModalVisible(false);
        }}
        footer={null}
      >
        <Row justify='center'>Create post</Row>
        <Row style={body}>
          <Col>
            <img src={user_Image} style={userImage} />
          </Col>
          <Col style={{ marginLeft: '5px' }}>{owner_name}</Col>
        </Row>
        ...
        <Row justify='center' style={{ marginTop: '10px' }}>
          {isEdit ? (
            <Button
              type='primary'
              disabled={
                (text == textBeforeEdit && photo == photoBeforeEdit) ||
                (text == '' && photo == '')
              }
              block
              onClick={submit}
